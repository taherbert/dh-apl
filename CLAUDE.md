...
- **Always test against ALL builds.** The APL is shared by all talent builds. Verify the build roster covers all templates: `npm run roster show`. The roster is generated via `npm run roster generate` (cluster-based from SPEC_CONFIG). `iterate.js` requires the roster and refuses single-build mode. If a change helps some builds but hurts others, create a sub-action-list gated by talent/hero-tree check, then re-test against the full roster.
