...
- **Always test against ALL archetypes.** The APL is shared by all talent builds. Verify the build roster covers all archetypes: `npm run roster show`. The roster is generated via `npm run roster generate` (after DoE discovery). `iterate.js` requires the roster and refuses single-build mode. If a change helps some archetypes but hurts others, create a sub-action-list gated by talent/hero-tree check, then re-test against the full roster.
