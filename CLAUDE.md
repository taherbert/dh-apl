...
- **Partial gains are NOT rejects — they are gating opportunities.** When a change helps some builds (+1% or more) but hurts others, this is a branching signal, not a rejection. Identify the discriminator (talent, apex rank, hero tree, cluster presence) and create a gated condition so the change applies only to builds that benefit. Then re-test the gated version against the full roster. Only reject a hypothesis if NO subset of builds benefits meaningfully, or if no valid SimC expression can discriminate the benefiting builds.
