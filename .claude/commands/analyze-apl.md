Walk through the VDH APL and explain it — what every line does, why it's there, and how the pieces fit together as a rotation.
...
1. Read the APL to explain. Use `$ARGUMENTS` if provided, else `apls/vengeance.simc`, else `apls/baseline.simc`.

2. Read spell data for context:
...
3. Read accumulated findings for calibration:

```
results/findings.json
```

4. Check for recent sim results (`ls results/`) — cast count data grounds the explanation in what actually happens vs what the APL intends.
...
- What action lists exist? (default, ar, anni, cooldowns, etc.)
...
- **Resource impact:** how does casting this affect fury, fragments, cooldowns, and buff states?
...
- What happens during burst windows? (Brand, Meta — does the APL shift behavior?)
...
default → [precombat] → [externals] → hero tree branch
  ├── ar → [cooldowns] → [core priority]
  └── anni → [cooldowns] → [core priority]
```
